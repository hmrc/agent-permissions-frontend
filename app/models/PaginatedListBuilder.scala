...
object PaginatedListBuilder {
  ...
  def build[T](page: Int, pageSize: Int, fullList: Seq[T]): models.PaginatedList[T] = {
    ...
    val pageSliceUntil = Math.min(pageEnd, if numberOfPages == page then fullList.length else fullList.length - 1)
