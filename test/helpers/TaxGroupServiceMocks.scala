...
trait TaxGroupServiceMocks extends AnyWordSpec with MockFactory {
  ...
  def expectCreateTaxGroup(arn: Arn)(implicit taxGroupService: TaxGroupService): Unit =
    (taxGroupService
      .createGroup(_: Arn, _: CreateTaxServiceGroupRequest)(using _: RequestHeader, _: ExecutionContext))
      .expects(arn, *, *, *)
  ...
  def expectUpdateTaxGroup(id: GroupId, payload: UpdateTaxServiceGroupRequest)(implicit
    ...
    (taxGroupService
      .updateGroup(_: GroupId, _: UpdateTaxServiceGroupRequest)(using _: RequestHeader, _: ExecutionContext))
      .expects(id, payload, *, *)
      .returning(Future.successful(Done))
      .once()
