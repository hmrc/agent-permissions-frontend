...
class AddClientsToGroupFormSpec extends AnyWordSpec with Matchers with GuiceOneAppPerSuite {
  ...
  "CreateGroupFrom binding" should {
    ...
    val encode: DisplayClient => String =
      client => Base64.getEncoder.encodeToString(Json.toJson(client).toString.getBytes)

    "be fillable with a AddClientsToGroup" in {
      val validatedForm = AddClientsToGroupForm
        .form()
        .fill(AddClientsToGroup(hasSelectedClients = false, None, None, Some(List(client1, client2))))
      validatedForm.hasErrors shouldBe false
      validatedForm.value shouldBe Option(
        AddClientsToGroup(hasSelectedClients = false, None, None, Some(List(client1, client2)))
      )
    }
    ...
    "be successful when button is Filter and form is non-empty" in {
      ...
      boundForm.value shouldBe Some(
        AddClientsToGroup(hasSelectedClients = false, None, Some("abc"), Some(List(client1, client2)))
      )
    }

    "have errors when clients is empty and hiddenClients is false" in {
      val params =
        Map(hasSelectedClients -> List("false"), search -> List.empty, filter -> List.empty, clients -> List.empty)
      val boundForm = AddClientsToGroupForm.form().bindFromRequest(params)
