    ...
    isAuthorisedAgent { arn =>
      isOptedInWithSessionItem[String](GROUP_NAME)(arn) { maybeName =>
        maybeName.fold(Redirect(routes.GroupController.showGroupName).toFuture) { name =>
          Ok(confirm_group_name(YesNoForm.form("group.name.confirm.required.error"), name)).toFuture
        }
      }
    }
  }
                  ...
                  agentPermissionsConnector
                    ...
                    .flatMap(nameAvailable =>
                      if (nameAvailable)
                        ...
                        Redirect(routes.GroupController.showAccessGroupNameExists).toFuture
                    )
                else
          ...
          isOptedInWithSessionItem[Seq[DisplayClient]](FILTERED_CLIENTS)(arn) { maybeFilteredResult =>
            isOptedInWithSessionItem[Boolean](HIDDEN_CLIENTS_EXIST)(arn) { maybeHiddenClients =>
              if (maybeFilteredResult.isDefined)
                Ok(
                  client_group_list(maybeFilteredResult, groupName, maybeHiddenClients, AddClientsToGroupForm.form())
                ).toFuture
              else
                groupService.getClients(arn).flatMap { maybeClients =>
                  Ok(
                    client_group_list(maybeClients, groupName, maybeHiddenClients, AddClientsToGroupForm.form())
                  ).toFuture
                }
            }
          }
            ...
            someMap
              .getOrElse(
                ...
                someMap.getOrElse(
                  "submitFilter",
                  someMap
                    .getOrElse("submitClear", throw new RuntimeException("invalid button value for submitAddClients"))
                )
              )
              .last
          )
            ...
            isOptedInWithSessionItem[Boolean](HIDDEN_CLIENTS_EXIST)(arn) { maybeHiddenClients =>
              AddClientsToGroupForm
                ...
                .fold(
                  formWithErrors =>
                    for {
                      ...
                      result <-
                        if (maybeFilteredResult.isDefined)
                          Ok(
                            client_group_list(maybeFilteredResult, groupName, maybeHiddenClients, formWithErrors)
                          ).toFuture
                        else
                          groupService.getClients(arn).flatMap { maybeClients =>
                  ...
                  formData =>
                    groupService
                      .processFormDataForClients(buttonSelection)(arn)(formData)
                      .map(_ =>
                        if (buttonSelection == ButtonSelect.Continue)
                          Redirect(routes.GroupController.showReviewSelectedClients)
                        else
                          Redirect(routes.GroupController.showSelectClients)
                      )
                )
            }
          }
                          ...
                          for {
                            ...
                            result <-
                              if (maybeFilteredResult.isDefined)
                                ...
                                Ok(
                                  team_members_list(maybeTeamMembers, groupName, maybeHiddenTeamMembers, formWithErrors)
                                ).toFuture
                              }
                          } yield result,
                        formData =>
                          groupService
                            .processFormDataForTeamMembers(buttonSelection)(arn)(formData)
                            .map(_ =>
                              if (buttonSelection == ButtonSelect.Continue)
    ...
    isAuthorisedAgent { arn =>
      isOptedInWithSessionItem[String](GROUP_NAME)(arn) { maybeGroupName =>
        isOptedInWithSessionItem[Seq[TeamMember]](GROUP_TEAM_MEMBERS_SELECTED)(arn) { maybeTeamMembers =>
          maybeGroupName.fold(Redirect(routes.GroupController.showGroupName).toFuture)(groupName =>
            maybeTeamMembers.fold(Redirect(routes.GroupController.showSelectTeamMembers).toFuture)(teamMembers =>
          ...
          val createGroupResponse = for {
            ...
            members: Option[Seq[AgentUser]] <- sessionCacheRepository
                                                 ...
                                                 .flatMap { maybeTeamMembers: Option[Seq[TeamMember]] =>
                                                   Future(
                                                     maybeTeamMembers
                                                       .map(tms => tms.map(tm => AgentUser(tm.userId.get, tm.name)))
                                                   )
                                                 }
