...
class CreateGroupSelectGroupTypeControllerSpec extends BaseSpec {
  ...
  "showSelectGroupType" should {
    "render correctly and clear existing session keys" in {
      ...
      implicit val request: FakeRequest[AnyContentAsEmpty.type] =
        FakeRequest("GET", ctrlRoute.showSelectGroupType().url)
      ...
      radios
        .select("#answer-item-hint")
        .text() shouldBe "Select clients based on any criteria. You can have a total of 1,200 clients and team members in a custom access group."
      radios
        .select("#answer-no-item-hint")
  ...
  "submitSelectGroupType" should {
    ...
    "render errors when no radio selected" in {
      ...
      implicit val request: FakeRequest[AnyContentAsFormUrlEncoded] =
        FakeRequest("POST", ctrlRoute.submitSelectGroupType().url)
          .withSession(SessionKeys.sessionId -> "session-x")
      ...
      html
        ...
        .text() shouldBe "Select the type of access group you want to create"
      html
        .select(Css.errorForField("answer"))
        .text() shouldBe "Error: Select the type of access group you want to create"
    }
    ...
    "redirect when answer is 'false'/'Access group based on tax service'" in {
      ...
      redirectLocation(result) shouldBe Some(
        controllers.routes.CreateGroupSelectGroupTypeController.showSelectTaxServiceGroupType().url
      )
    }
  }
  ...
  "showSelectTaxServiceGroupType" should {
    ...
    "render select_group_tax_type correctly" in {
      ...
      implicit val request: FakeRequest[AnyContentAsEmpty.type] =
        FakeRequest("GET", ctrlRoute.showSelectTaxServiceGroupType().url)
          .withSession(SessionKeys.sessionId -> "session-x")
      ...
      html
        .select("#tsg-inset")
        .text() shouldBe "We will add new clients to this group automatically. We’ll do this when they authorise you for the selected tax service. You can manually remove specific clients later, using the ‘Manage access groups’ section."
  ...
  "submitSelectTaxServiceGroupType" should {
    ...
    "render errors when empty form submitted" in {
      ...
      implicit val request: FakeRequest[AnyContentAsFormUrlEncoded] =
        FakeRequest("POST", ctrlRoute.submitSelectTaxServiceGroupType().url)
          .withSession(SessionKeys.sessionId -> "session-x")
  ...
  "showReviewTaxServiceGroupType" should {
    ...
    "render correctly" in {
      ...
      implicit val request: FakeRequest[AnyContentAsEmpty.type] =
        FakeRequest("GET", ctrlRoute.showReviewTaxServiceGroupType().url)
          .withSession(SessionKeys.sessionId -> "session-x")
  ...
  "submitReviewTaxServiceGroupType" should {
    ...
    "render errors when empty form submitted" in {
      ...
      implicit val request: FakeRequest[AnyContentAsEmpty.type] =
        FakeRequest("POST", ctrlRoute.submitReviewTaxServiceGroupType().url)
          .withSession(SessionKeys.sessionId -> "session-x")
      ...
      html
        .select(Css.errorSummaryForField("answer"))
        .text() shouldBe "Select yes if you would like to create a group of this type"
      html
        .select(Css.errorForField("answer"))
        .text() shouldBe "Error: Select yes if you would like to create a group of this type"
    }
