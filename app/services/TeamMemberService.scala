...
class TeamMemberServiceImpl @Inject() (
    ...
    teamMembersInSession.flatMap(_ =>
      formData.submit.trim match {
        case CONTINUE_BUTTON =>
          sessionCacheService.deleteAll(teamMemberFilteringKeys).flatMap(_ => teamMembersInSession)
  ...
  def getPageOfTeamMembers(arn: Arn)(page: Int = 1, pageSize: Int = 10)(implicit
    ...
    for {
      ...
      paginationMetaData = PaginationMetaData(
        ...
        extra = Some(
          Map("totalSelected" -> JsNumber(totalMembersSelected))
  ...
  def lookupTeamMembers(
    arn: Arn
  )(ids: Option[List[String]])(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[List[TeamMember]] =
    ids.fold(List.empty[TeamMember].toFuture) { ids =>
      getTeamMembersFromConnector(arn).map(tms => tms.filter(tm => ids.contains(tm.id)).toList)
    }
