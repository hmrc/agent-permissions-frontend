...
/** The function in this class will take care of handling the submission (post) of a page with both pagination and
  * filtering present. This is an abstract class as the user must provide implementations for some page-specific pieces
  * of functionality. The following conventions should be respected:
  *   - The form should have at minimum the following values: * "search" (optional) * "filter" (optional) * "submit"
  *     (mandatory)
  *   - The submit button will indicate what input the user has given: * "continue": the user intends to continue the
  *     journey onto the next page * "filter": the user wishes to apply filters to the current view according to their
  *     form input * "clear": the user wishes to clear all current filters * "pagination_[x]": the user wishes to view
  *     page number 'x' of the view.
...
abstract class POSTPaginationHandler[FormData](sessionCacheService: SessionCacheService) {
  ...
  /** HTTP call used by the page to reload itself, if it should need to. The parameters are page number, search term,
    * filter term
    */
  val reloadCall: (Option[Int], Option[String], Option[String]) => Call

  /** A function that will be called when submitting with a valid form, regardless of which button the user has
    * submitted with. Typically used to update the session state in some way.
    */
...
/** Same as [[POSTPaginationHandler]] but also takes care of remembering selected items while browsing through different
  * pagination pages, and when reloading the page by setting and unsetting filters. filtering present. The following
  * conventions should be respected. The currently selected items are saved in the session value "SELECTED_CLIENTS".
  */
abstract class POSTPaginatedSearchableClientSelectHandler(
  ...
  def handlePost(implicit ec: ExecutionContext, request: Request[_], formBinding: FormBinding): Future[Result] =
    new POSTPaginationHandler[AddClientsToGroup](sessionCacheService) {
      ...
      val onSubmit: AddClientsToGroup => Future[Unit] = formData =>
        sessionCacheOps.savePageOfClientsForCreateGroup(formData).map(_ => ())
